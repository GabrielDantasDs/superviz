...
export default class CardRepository {
  ...
  private insertCard(projectId: number, listId: number, card: Card): Promise<void> {
    return new Promise((resolve, reject) => {
      this.db.getConnection().run(
        `INSERT INTO cards (id_project, title, position, content, id_list) VALUES (?, ?, ?, ?, ?)`,
        [projectId, card.title, card.position, card.content, listId],
        (err) => {
          if (err) reject(err);
          else resolve();
        }
  ...
  async updateCard(cardId:Number, card:Card): Promise<any> {
    return new Promise((resolve, reject) => {
      this.db.getConnection().serialize(() => {
        this.db.getConnection().run(`UPDATE cards set title = ?,content = ? where id = ?`, [card.title, card.content, cardId], async (err) => {
          if (err) reject(err)
          else {
            this.db.getConnection().get(`SELECT * FROM cards where id = ?`, [cardId], (err, row) => {
              if (err) reject(err);
              else resolve(row);
            })
          }
